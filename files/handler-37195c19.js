import * as require$$3 from 'fs';
import require$$3__default, { readdirSync, statSync } from 'fs';
...
function viaLocal(dir, isEtag, uri, extns) {
	...
	for (; i < arr.length; i++) {
		...
		if (abs.startsWith(dir) && require$$3.existsSync(abs)) {
			stats = require$$3.statSync(abs);
...
function send(req, res, file, stats, headers) {
	...
	require$$3.createReadStream(file, opts).pipe(res);
}
...
class Body {
	...
	async formData() {
		...
		const {toFormData} = await import('./multipart-parser-4fac399d.js');
...
function serve(path, max_age) {
	return (
		require$$3__default.existsSync(path) &&
		sirv(path, {
			etag: true,
			maxAge: max_age,
			immutable: true,
